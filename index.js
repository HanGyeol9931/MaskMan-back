  ...
  await user
    ...
    .then((e) => {
      if (e == null) {
        ...
        nft
          .findAll({ raw: true, where: { nft_owner: e.user_nickname } })
          .then((mynft) => {
...
app.post("/bordercreact", (req, res) => {
  ...
  border
    .create({
      ...
      border_writer: nickname,
      border_writer_profil: img,
    })
    .then(() => {
      border
        .findAll({
...
app.post("/replie", async (req, res) => {
  ...
  await replies
    .create({
      border_id: borderid,
      replies_writer: nickname,
      replies_profil: profil,
...
app.post("/minting", async (req, res) => {
  ...
  await user
    .update({ user_point: point }, { where: { user_nickname: nickname } })
    .then(() => {
      nft
        .update({ nft_owner: nickname }, { where: { id: minting_id } })
        .then(() => {
          user
            .findOne({
              raw: true,
            ...
            .then((user) => {
              nft
                ...
                .then((nftdata) => {
                  nft
                    .findAll({ raw: true, where: { nft_owner: "null" } })
                    .then((nftcount) => {
                      nft
                        .findAll({ raw: true, where: { nft_owner: nickname } })
                        .then((mynft) => {
                          res.send({
                            user: user,
                            nft: nftdata,
...
app.post("/nftsell", async (req, res) => {
  ...
  await shop
    .create({
      ...
      shop_img: img,
      shop_title: name,
      shop_writer: nickname,
      shop_writer_profil: user_profil,
    })
    .then(() => {
      nft.update(
        {
          ...
          shop.findAll({ raw: true }).then((nftsell) => {
            nft.findAll({ raw: true }).then((nfts) => {
              nft
                ...
                .then((mynft) => {
                  user
                    .findOne({ raw: true, where: { user_nickname: nickname } })
                    .then((user) => {
                      res.send({
                        nftsell: nftsell,
                        nft: nfts,
...
app.post("/nftstaking", async (req, res) => {
  ...
  await nft
    .update({ nft_staking: 1 }, { where: { nft_tilte: name } })
    .then(() => {
...
app.post("/stakingPoint", async (req, res) => {
  ...
  await nft
    .update({ nft_stakingPoint: 0 }, { where: { nft_owner: nickname } })
    .then(() => {
      user.update(
        { user_point: Number(userpoint) + Number(stakingpoint) },
        { where: { user_nickname: nickname } }
      );
      nft.findAll({ where: { nft_owner: nickname } }).then((e) => {
        user
          ...
          .then(() => {
            user
              ...
              .then((users) => {
                nft
                  .update(
                    { nft_staking: 0, nft_stakingPoint: 0 },
                    { where: { nft_tilte: name } }
                  )
                  .then(() => {
                    nft
                      .findAll({ where: { nft_owner: nickname } })
                      .then((mynft) => {
...
app.post("/imgupload", uploads.upload.single("files"), (req, res) => {
  ...
  user
    ...
    .then(() => {
      user
        .findOne({ raw: true, where: { user_nickname: formData.nickname } })
        .then((e) => {
          res.send({ user: e });
        });
    })
...
app.post("/userschange", (req, res) => {
  ...
  if (type === "일반유저") {
    user
      .update({ user_whitelist: 0 }, { where: { user_nickname: nickname } })
      .then(() => {
        user.findAll({ raw: true }).then((e) => {
          res.send({ users: e });
        });
    ...
    user
      .update({ user_whitelist: 1 }, { where: { user_nickname: nickname } })
      .then(() => {
        user.findAll({ raw: true }).then((e) => {
          res.send({ users: e });
        });
    ...
    user
      .update({ user_stop: 1 }, { where: { user_nickname: nickname } })
      .then(() => {
        user.findAll({ raw: true }).then((e) => {
          res.send({ users: e });
        });
    ...
    user
      .update({ user_stop: 0 }, { where: { user_nickname: nickname } })
      .then(() => {
        user.findAll({ raw: true }).then((e) => {
          res.send({ users: e });
        });
    ...
    user
      .update({ user_admin: 1 }, { where: { user_nickname: nickname } })
      .then(() => {
        user.findAll({ raw: true }).then((e) => {
          res.send({ users: e });
        });
    ...
    user
      .update({ user_admin: 0 }, { where: { user_nickname: nickname } })
      .then(() => {
        user.findAll({ raw: true }).then((e) => {
          res.send({ users: e });
        });
